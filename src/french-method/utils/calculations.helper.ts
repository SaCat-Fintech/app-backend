...
export const calculateAnnualEffectiveRate = (
  rate_type: RateType,
  rate_value: number,
  days_per_year: number,
  capitalization_period_days: number,
): number => {
  let result = 0;
  ...
  if (rate_type === "TNA") {
    result =
      Math.pow(
        1 + rate_value / (days_per_year / capitalization_period_days),
        days_per_year / capitalization_period_days,
      ) - 1;
  } else {
...
export const calculateEffectiveRateByPaymentFrequency = (
  ...
  return (
    Math.pow(
      1 + annual_effective_rate,
      payment_frequency_days / days_per_year,
    ) - 1
  );
};
